...
public class TransactionService {
    ...
    public TransactionDto useBalance(Long userId, String accountNumber, Long amount) {
        ...
        //노출됨..-> 엔티티 클래스 안에다가 구현하고 호출하기 -> 다 처리하고 엔티티에 직접 반엉하도록
        /*
        Long accountBalance = account.getBalance();
        account.setBalance(accountBalance - amount);
        */
        //문제가 생겼을때 다음 코드로 진행한되도록 트렌젝션 적용. 서비스에 달아도되고(모든 서비스내의 내용에 적용). 매서드에 달아도됨(해당매서드에적용)..서비스에선 보통 안전하게 다 달아줌
    ...
    private Transaction saveAndGetTransaction(
        ...
        return transactionRepository.save(
                Transaction.builder()
                        ...
                        .transactionId(UUID.randomUUID().toString().replace("-", "")) //1.고유한값 2.순사와 상관없이 랜덤하게 -> UUID사용
